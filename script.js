...
document.getElementById('closeButton').addEventListener('click', function() {
  // Nasconde il riquadro informativo e mostra il pulsante "?"
  document.getElementById('infoBox').classList.add('hidden');
  document.getElementById('toggleInfoBox').classList.remove('hidden');
});

document.getElementById('toggleInfoBox').addEventListener('click', function() {
  // Mostra il riquadro informativo e nasconde il pulsante "?"
  document.getElementById('infoBox').classList.remove('hidden');
  document.getElementById('toggleInfoBox').classList.add('hidden');
});
...
function aggiornaSpecificheTable() {
    ...
    const GluPerc = parseFloat(document.getElementById('GluPerc').value) || 0;
...
function mantenimentoMeql() {
    ...
    const mgkgminmantenimento = (mantenimentoMl * 10 * GluPerc) / WW / 1440;
    ...
    const mgkgmin24ore = ((SommaDie24OreMl * 10 * GluPerc) / WW / 1440) + mgkgminmsoluzione + mgkgminliquidiaggiunti;
    ...
    const mgkgminxore = ((SommaDiexOreMl * 10 * GluPerc) / WW / 1440) + mgkgminmsoluzione + mgkgminliquidiaggiunti;
...
// roba singola infusione
    ...
    const mlNaClOutputValue = naOutputValue / 4;
    ...
    if (tableRows.length > 5) {
        tableRows[5].cells[2].innerHTML = `
            <div style="display: flex; justify-content: space-between;">
                <div>${SodioDaPerdereRecuperare.toFixed(1)}</div>
                <div>${CalcoloAggiuntivo.toFixed(1)}</div>
            </div>
        `;
    }
...
document.getElementById('GluPerc').addEventListener('input', aggiornaSpecificheTable);
